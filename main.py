...
class GeminiTTSGenerator(Star):
    ...
    def _load_plugin_config(self):
        default_config = {"group_tts_settings": {}}
        if not os.path.exists(self.config_path):
            self.group_tts_settings = default_config["group_tts_settings"]
            self._save_plugin_config()
        else:
            with open(self.config_path, "r", encoding="utf-8") as f:
                try:
                    plugin_conf = json.load(f)
                    self.group_tts_settings = plugin_conf.get("group_tts_settings", {})
                    ...
                    self.group_tts_settings = default_config["group_tts_settings"]
                    self._save_plugin_config()
        logger.info(f"群组配置已加载: {len(self.group_tts_settings)} 个群聊有独立设置")

    def _save_plugin_config(self):
        config_data = {"group_tts_settings": self.group_tts_settings}
    ...
    async def toggle_auto_tts(self, event: AstrMessageEvent, switch: str):
        ...
        if not group_id:
            ...
            return
        if switch.lower() == "on":
            self.group_tts_settings[group_id] = True
            yield event.plain_result("本群的自动语音功能已开启。")
        elif switch.lower() == "off":
            self.group_tts_settings[group_id] = False
            yield event.plain_result("本群的自动语音功能已关闭。")
        else:
    ...
    async def on_llm_reply_to_tts(self, event: AstrMessageEvent):
        try:
            ...
            is_tts_enabled_for_group = self.group_tts_settings.get(group_id, False)
            if not (
                is_tts_enabled_for_group
            ...
            if len(chain) == 1 and isinstance(chain[0], Comp.Plain):
                ...
                logger.info(f"群聊 {group_id} 触发自动语音流程。")
                ...
                try:
                    ...
                    if audio_data:
                        ...
                        if save_path:
                            logger.info(
                                "【TTS插件-发送准备】语音已生成。准备构建 Comp.Record 对象。"
                            )
                            record_component = Comp.Record(file=save_path)
                            chain.append(record_component)
                            logger.info(
                                "【TTS插件-发送准备】Comp.Record 已成功添加到消息链，等待框架发送。"
                            )
                        else:
                            logger.error(
                                "音频数据已生成，但保存为WAV文件时失败。无法发送语音。"
                            )
                    else:
